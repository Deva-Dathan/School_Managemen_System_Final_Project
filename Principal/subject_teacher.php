    ...
    <ul class="nav-links" id="nav-links">
        ...
        <li>
          <a href="class_teacher.php">
            ...
            <span class="links_name">Class Teacher</span>
          </a>
        </li>
        <li>
          <a href="subject_teacher.php" class="active">
          <i class='bx bx-book-alt' style="color:var(--light);"></i>
            <span class="links_name" style="color:var(--light);">Subject Teacher</span>
          </a>
        </li>
        <li>
  ...
  <section class="home-section">
    ...
    //   error_reporting(0);
      if($_SERVER["REQUEST_METHOD"] == "POST")
      {
        ...
        $ct_sql = "SELECT * FROM subject_teacher WHERE teacher_name = '$teacher_name' AND u_subject = '$subject' AND standard = '$standard'";
        $result = $conn->query($ct_sql);
        
        if ($result->num_rows > 0) {
            // Subject already exists, show an error message or handle it accordingly
            echo '<script>Swal.fire({icon: "error",title: "Oops...",text: "Already assigned as Subject Teacher!"});</script>';
        } else {
            // Email does not exist, proceed with the insertion
            $sql = "INSERT INTO subject_teacher (teacher_name, u_email, u_subject, standard) VALUES ('$teacher_name', '$email', '$subject', '$standard')";
            if ($conn->query($sql) === TRUE) {
              echo '<script>Swal.fire({title: "SUBJECT TEACHER ASSIGNED",text: "Assigning Successfully",icon: "success"});</script>';
            } else {
...
<div class="row">
  ...
  <label for="validationCustom01" class="required">Teacher</label>
              <select class="custom-select" name="teacher_name" id="validationCustom01" required>
                ...
                <?php
          include("../include/db_connection.php");
          $sql = "SELECT * FROM users INNER JOIN teachers_data ON users.u_email = teachers_data.u_email WHERE users.u_role IN ('TEACHER', 'CLASS TEACHER')";
...
$dis_subject = "SELECT * FROM subject_data";
$sub_result = $conn->query($dis_subject);
...
?>
  ...
  <label for="validationCustom04" class="required">Standard</label>
        ...
        <button type="submit" name="u_assign" class="btn btn-success">Assign Subject Teacher To Class</button> 
      </form>
...
?>
            ...
            <tr align=center>
              ...
              <td><?php echo $row['u_subject'];?></td>
              <td><?php echo $row['standard'];?></td>
              <td><a href="subject_teacher_delete.php?u_email=<?php echo $row['u_email'];?>"><button class="btn btn-danger"><i class="bx bxs-trash"></i></button></a></td>
            </tr>
