...
<body id="element">
  ...
  <div class="sidebar" id="sidebar">
    ...
    <ul class="nav-links" id="nav-links">
        ...
        <li>
          <a href="class_teacher.php" class="active">
            <i class='bx bx-user-circle' style="color:var(--light);"></i>
            <span class="links_name" style="color:var(--light);">Class Teacher</span>
          </a>
        </li>
        <li>
          <a href="subject_teacher.php">
  ...
  <section class="home-section">
    ...
    <div class="home-content">
      <div class="sales-boxes">
        <div class="top-sales box">
          <div class="row">
          <div class="col-md-6 col-12 title">VIEW TEACHERS</div>
    ...
    //   error_reporting(0);
      if($_SERVER["REQUEST_METHOD"] == "POST")
      {
        ...
        $ct_sql = "SELECT * FROM class_teacher WHERE teacher_name = '$teacher_name' AND standard = '$standard'";
        $result = $conn->query($ct_sql);
        
        if ($result->num_rows > 0) {
            // Subject already exists, show an error message or handle it accordingly
            echo '<script>Swal.fire({icon: "error",title: "Oops...",text: "Already assigned as Class Teacher!"});</script>';
        } else {
            // Email does not exist, proceed with the insertion
            $sql = "INSERT INTO class_teacher (teacher_name, standard, u_email) VALUES ('$teacher_name', '$standard', '$email')";
            if ($conn->query($sql) === TRUE) {
                $update_query = "UPDATE users SET u_role='CLASS TEACHER' WHERE u_email='$email'";
                $result1 = $conn->query($update_query);
                echo '<script>Swal.fire({title: "CLASS TEACHER ASSIGNED",text: "Assigning Successfully",icon: "success"});</script>';
            } else {
...
<div class="row">
  ...
  <label for="validationCustom01" class="required">Teacher</label>
              <select class="custom-select" name="teacher_name" id="validationCustom01" required>
          ...
          {
            ?>
                <option value="<?php echo $row['u_name'];?>"><?php echo $row['u_name']." (".$row['u_qualification'].")";?></option>
                <?php
          }
          ?>
              </select>
              <div class="invalid-feedback">Please select a teacher from the list.</div>
  </div> <!-- col-md-6 mb-3 close -->

  <script>
    // Using jQuery for simplicity, you can use vanilla JavaScript if preferred
    $(document).ready(function(){
        $('#validationCustom01').change(function(){
            var teacherName = $(this).val();
            $.ajax({
                type: 'POST',
                url: 'get_subject.php', // PHP script to fetch email based on faculty name
                data: {teacher_name: teacherName},
                success: function(response){
                    $('#validationCustom02').val(response);
                }
            });
        });
    });
  ...
  <div class="col-md-6 mb-3">
<label for="validationCustom02" class="required">Subject</label>
            <input type="text" class="form-control" name="teacher_subject" id="validationCustom02" required>
...
<script>
    ...
    $(document).ready(function(){
        $('#validationCustom01').change(function(){
            ...
            $.ajax({
                type: 'POST',
                url: 'get_email.php', // PHP script to fetch email based on faculty name
                data: {teacher_name: teacherName},
                success: function(response){
                    $('#validationCustom03').val(response);
                }
            });
        });
    });
...
<label for="validationCustom03" class="required">E-Mail</label>
            <input type="text" class="form-control" name="e_mail" id="validationCustom03" required>
            <div class="valid-feedback">Looks good!</div>
</div>

<?php

$dis_cls = "SELECT * FROM class_teacher";
$new_result = $conn->query($dis_cls);

$db_classes = array();
if ($new_result->num_rows > 0) {
    while ($row = $new_result->fetch_assoc()) {
        $db_classes[] = $row["standard"];
    }
}
// Array of all possible classes
$all_classes = array("I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X");

// Remove classes already stored in the database
$remaining_classes = array_diff($all_classes, $db_classes);

?>

  <div class="col-md-6 mb-3">
  <label for="validationCustom04" class="required">Standard</label>
              <select class="custom-select" name="standard" id="validationCustom04" required>
              <option selected disabled>Choose...</option>
            <?php foreach ($all_classes as $class): ?>
                <?php if (!in_array($class, $db_classes)): ?>
        ...
        <button type="submit" name="u_assign" class="btn btn-success">Assign Class Teacher</button>
        </form>
...
?>
            ...
            <tr align=center>
              ...
              <td><a href="class_teacher_delete.php?u_email=<?php echo $row['u_email'];?>"><button class="btn btn-danger"><i class="bx bxs-trash"></i></button></a></td>
            </tr>
...
{
  echo '<div class="alert alert-danger text-center font-weight-bold" role="alert">NO CLASS TEACHER RECORD FOUND....!</div>';
}
?>
        </tbody>
    </table> 
