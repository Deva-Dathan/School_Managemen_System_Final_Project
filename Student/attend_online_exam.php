...
<div class="container p-lg-4">
    ...
    <form class="mt-5 pt-3" method="post" action="submit_answer_process.php" id="myForm">
...
$exam_id = $conn->real_escape_string($_GET['exam_id']);
$sql = "SELECT * FROM exam_questions WHERE online_exam_id = '$exam_id'";
...
<div class="form-row mt-5 col-12">
    ...
    <div class="col-md-5 ml-4 mt-3 form-check"><input type="radio" name="answer<?php echo $i ?>"
            class="form-check-input" value="<?php echo $user_data['option_a'];?>" id="optiona<?php echo $i ?>"><label class="form-check-label"
            for="optiona<?php echo $i ?>"><?php echo $user_data['option_a'] ?></label></div>
    <div class="col-md-5 ml-4 mt-3 form-check"><input type="radio" name="answer<?php echo $i ?>"
            class="form-check-input" value="<?php echo $user_data['option_b'];?>" id="optionb<?php echo $i ?>"><label class="form-check-label"
            for="optionb<?php echo $i ?>"><?php echo $user_data['option_b'] ?></label></div>
    <div class="col-md-5 ml-4 mt-3 form-check"><input type="radio" name="answer<?php echo $i ?>"
            class="form-check-input" value="<?php echo $user_data['option_c'];?>" id="optionc<?php echo $i ?>"><label class="form-check-label"
            for="optionc<?php echo $i ?>"><?php echo $user_data['option_c'] ?></label></div>
    <div class="col-md-5 ml-4 mt-3 form-check"><input type="radio" name="answer<?php echo $i ?>"
            class="form-check-input" value="<?php echo $user_data['option_d'];?>" id="optiond<?php echo $i ?>"><label class="form-check-label"
...
// Assuming $conn is your database connection
$sql = "SELECT * FROM exam_questions WHERE online_exam_id = '".$_GET['exam_id']."'";
$result = $conn->query($sql);

if ($result && $result->num_rows > 0) {
    while ($user_data3 = $result->fetch_assoc()) {
        // Accessing the fields
        $time = $user_data3['exam_duration'];
        
        // Define the function to calculate seconds from time
        ...
        // Call the function to get seconds from time
        $time = seconds_from_time($time);

        // Further processing if needed
    }
} else {
    echo "No results found."; // Handle the case where no results are found
}
?>
